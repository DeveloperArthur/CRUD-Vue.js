...
export default {
  ...
  methods: {
    adicionar() {
      if (
        this.nomeDepartamento.length == 0 ||
        this.descricaoDepartamento.length == 0
      ) {
        ...
        this.$store.state.departamentos.descricao.push(
      ...
      if (this.pesquisa.length == 0) {
        ...
        if (this.$store.state.departamentos.id.length == 0) {
          ...
          for (var i = 0; i < this.$store.state.departamentos.id.length; i++) {
            if (
              this.$store.state.departamentos.id[i] == this.pesquisa ||
    ...
    excluir(index) {
      for(var i = 0; i < this.$store.state.usuarios.id.length; i++){
        if(this.$store.state.usuarios.departamento[i] == this.$store.state.departamentos.nome[index]){
    ...
    alterar() {
      if (this.novoDepartamento.length == 0 || this.novaDescricao.length == 0) {
        ...
        for (var i = 0; i < this.$store.state.departamentos.id.length; i++) {
          if (
            this.$store.state.departamentos.nome[i] ==
              this.departamentoAtualAlterar &&
            this.$store.state.departamentos.descricao[i] ==
    ...
    mostrarTelaAlterarNaBusca() {
      ...
      for (var i = 0; i < this.$store.state.departamentos.id.length; i++) {
        if (
          ...
          this.departamentoAtualAlterar = this.$store.state.departamentos.nome[
            i
          ];
          this.descricaoAtualAlterar = this.$store.state.departamentos.descricao[
            i
          ];
        }
      }
    ...
    excluirNaPesquisa() {
      for(var i = 0; i < this.$store.state.usuarios.id.length; i++){
        if(this.$store.state.usuarios.departamento[i] == this.$store.state.departamentos.nome[this.pesquisaId - 1]){
          alert('ERRO: Usuarios estao vinculados a este departamento');
          return 0;
        }
      }
